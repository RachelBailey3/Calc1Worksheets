...
\begin{enumerate}
\item $\displaystyle\frac{\ln(x-2)}{x^2-6x-7}=\answer{(2,7)}\cup \answer{(7,\infty)}$
...
\[
x=\answer{\frac{\pi}{3}, \frac{2\pi}{3}, \frac{4\pi}{3}, \frac{5\pi}{3}}
\]
...
\\
For $f(x)$ to be continuous at $x=2$, we need $\lim\limits_{x\to \answer{2}}f(x)=f(\answer{2})$, thus, if we can define $f(2)=\answer{\frac{3}{2}}$, $f(x)$ will be continuous at $x=2$.
